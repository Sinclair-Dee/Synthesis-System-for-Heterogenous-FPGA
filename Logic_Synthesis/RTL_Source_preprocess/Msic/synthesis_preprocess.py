...
def merge1():
    ...
    #合成到cnn_top.v中
    outfile = open("cnn_top1.v","w",encoding='utf-8')
...
def merge2():
    outfile  = open("cnn_top2.v","w",encoding='utf-8')
    #打开生成的cnn_top1
    infile = open("cnn_top1.v","r",encoding='utf-8')
    for line in infile:
        if line.count('\n') == len(line):
            pass
        else:            
...
def remove():
    outfile  = open("cnn_top3.v","w",encoding='utf-8')
    #打开生成的cnn_top2
    infile = open("cnn_top2.v","r",encoding='utf-8')
...
def matr():#目前只有识别的功能，自动展开待做。
    outfile  = open("cnn_top4.v","w",encoding='utf-8')
    #打开生成的cnn_top3
    infile = open("cnn_top3.v","r",encoding='utf-8')
...
def main():
	...
	remove_file1 = open('cnn_top.clk','r') #删除其他的文件。
	remove_file1.close()
	remove_file2 = open('syn_st','r')
	remove_file2.close()
	remove_file3 = open('comet_pll.info','r')
	remove_file3.close()

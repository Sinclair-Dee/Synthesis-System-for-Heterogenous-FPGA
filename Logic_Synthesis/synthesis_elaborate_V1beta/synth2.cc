...
extern list<perm_string> clock_names;
#endif
...
#ifdef __VERIFICATION__
static void get_clock_name(NetEvProbe* pclk){
	assert(pclk->pin_count() == 1);
	Link& link = pclk->pin(0);
	Nexus* nexus = link.nexus();
	for (Link* idx = nexus->first_nlink();idx; idx = idx->next_nlink())
	{
		NetNet*net = dynamic_cast<NetNet*>(idx->get_obj());
		if (net && (net->port_type() == NetNet::PINPUT))
		{
			clock_names.push_back(net->name());
		}
	}
}
#endif
	...
	NetNet*accum_in)
{
	...
	for (NetAssign_*cur = lval_ ;  cur ;  cur = cur->more) {
		...
		if (cur->bmux() != 0) {
			//1.根据adr的位宽例化netdecoder;
			//2.assert(decoder.pin_result().width() >= nex_out.width());
			//3.为每个位宽产生选通器mux1_1_2，mux.data(0,0)连接到nex_map对应位宽上，
			//  mux.data(0,1)连接到右值信号上，mux.result(0)连接到nex_out对应位宽上；
			assert(sync_flag);
			
			NetNet*adr = cur->bmux()->synthesize(des);
			NetDecoder*decoder = new NetDecoder(scope, scope->local_symbol(), adr->pin_count(), (1 << adr->pin_count()));
			des->add_node(decoder);
			NetNet* decoder_out = new NetNet(scope,scope->local_symbol(),NetNet::WIRE,(1 << adr->pin_count()));
			for (unsigned idx = 0; idx < adr->pin_count();idx++){
				connect(decoder->pin_Data(idx), adr->pin(idx));
			}
			for (unsigned idx = 0; idx < (1 << adr->pin_count());idx++){
				connect(decoder->pin_Eq(idx), decoder_out->pin(idx));
			}
			
			NetNet* mux_out_bus = new NetNet(scope,scope->local_symbol(),NetNet::WIRE,lsig->pin_count());

			struct nexus_map_t*nex_map_idx = make_nexus_index(nex_map);	

			if(decoder_out->pin_count() < lsig->pin_count()){
				for (unsigned idx = 0; idx < decoder_out->pin_count(); idx++){
					int ptr = map_nexus_in_index(nex_map_idx,
						nex_map->pin_count(),
						lsig->pin(idx).nexus());

					NetMux *mux_1_1_2 = new NetMux(scope, scope->local_symbol(), 1, 2, 1);
					des->add_node(mux_1_1_2);
					connect(mux_1_1_2->pin_Data(0,0), nex_map->pin(ptr));
					connect(mux_1_1_2->pin_Data(0,1), rsig->pin(roff));
					connect(mux_1_1_2->pin_Result(0), mux_out_bus->pin(idx));
					connect(mux_1_1_2->pin_Sel(0), decoder_out->pin(idx));
				}

				for (unsigned jdx = decoder_out->pin_count(); jdx< lsig->pin_count(); jdx ++){
					int ptr = map_nexus_in_index(nex_map_idx,
						nex_map->pin_count(),
						lsig->pin(jdx).nexus());

					connect(nex_map->pin(ptr), mux_out_bus->pin(jdx));
				}
				

			}else{
				for (unsigned idx = 0; idx < lsig->pin_count(); idx++){
					int ptr = map_nexus_in_index(nex_map_idx,
						nex_map->pin_count(),
						lsig->pin(idx).nexus());
			...
			for (unsigned idx = 0; idx < mux_out_bus->pin_count(); idx++){
				int ptr = map_nexus_in_index(nex_map_idx,
					nex_map->pin_count(),
					lsig->pin(idx).nexus());

				connect(mux_out_bus->pin(idx), nex_out->pin(ptr));
			}
	...
	NetNet*nex_map, NetNet*nex_out, NetNet*accum_in)
{
	...
	do {
		...
		//ADU: 在时序always模块中,
		//     这里的nex_ff中包含的FF的个数与tmp引脚的个数可能不一致,例：
		/*
		always@(posedge clk)
			if(en1)
				begin
...
{	
	...
	for(unsigned item = 0 ;  item < nitems_ ;  item += 1){
		...
		if(items_[item].guard != 0){
			NetEConst*ge = dynamic_cast<NetEConst*>(items_[item].guard);/*类NetEConsParam继承类NetEConst——ADU*/
			if (ge == 0) {
		...
		while (! gstack.empty()) {
			...
			if (tmp.is_defined()
				|| type() == NetCase::EQ) {
					...
					statement_map[sel_idx] = items_[item].statement;/*选择端sel和数据端的对应关系——ADU*/
	...
	for (unsigned item = 0 ;  item < (1U<<sel_pins) ;  item += 1) {
		...
		/*如下例所示，当下面的case语句存在于组合always语句中时就会执行下面遇到statement_map[2] == 0的情况
		case({s1,s0})
		2'b00: y = a;
		2'b01: y = b;
		endcase
	...
	for (unsigned wdx = 0 ;  wdx < mux->width() ;  wdx += 1) {
		if(sync_flag){
			...
			//应该检测linked_count == 0的情况！！！！
			if(linked_count == 0){
				//有待处理
				//出现这中情况时，该MUX肯定不会跟输出端口直接相连
				//因此对应的该nexus_out不连接，留给上一级处理
				//assert(0);
				...
				for (unsigned item = 0 ; item < (1U<<sel_pins) ;  item += 1) {
					...
					if ( mux->pin_Data(wdx,item).is_linkedToSignificantSignal() ) {						
						...
						//让未连接的mux->pin_Data(wdx,item)接地——ADU
						NetConst*zero = new NetConst(scope, scope->local_symbol(),
							verinum::V0);
						zero->set_line(*this);
						des->add_node(zero);
	...
	for (unsigned idx = 0 ;  idx < nex_out->pin_count() ;  idx += 1) {
		...
		case 0010:
			if (sync_flag) {
				...
				//如果一条赋值语句在else中有而if中没有，这样产生的锁存器的控制信号需要添加一个非门——ADU
				/*---ADU
				always@(*)
				if(la)
				q1 = a;
				...
				*/
				...
				//两个gate之间的引脚不能直接相连，中间必须添加一条net，否则引脚pin中的nexus中的private_data为0——ADU
...
void synth2_f::process(class Design*des, class NetProcTop*top)
{
	...
	if (! top->is_asynchronous()) {
		...
		if (! synth_error_flag) {
			...
			if(NetProcTop::KINITIAL == top->type()){
				des->delete_process(top);
			}else{
				cerr << top->get_line() << ": warning: "
					<< "Process not synthesized." << endl;

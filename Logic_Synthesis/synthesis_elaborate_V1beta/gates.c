...
static void show_gate_lpm(ivl_lpm_t net)
{
	...
	/*begin-ADU*/
	case IVL_LPM_CMP_LE:
		if (device->show_cmp_le == 0) {
			fprintf(stderr, "fpga.tgt: IVL_LPM_CMP_LE not supported"
				" by this target.\n");
			return;
		}
		device->show_cmp_le(net);
		break;
	/*end-ADU*/

	/*begin-ADU*/
	case IVL_LPM_CMP_LT:
		if (device->show_cmp_lt == 0) {
			fprintf(stderr, "fpga.tgt: IVL_LPM_CMP_LT not supported"
				" by this target.\n");
			return;
		}
		device->show_cmp_lt(net);
		break;
	/*end-ADU*/
	...
	case IVL_LPM_FF:
		...
		/*start-------ADU*/
	case IVL_LPM_LATCH:
		if (device->show_lacth == 0) {
			fprintf(stderr, "fpga.tgt: IVL_LPM_LATCH not supported"
				" by this target.\n");
			return;
		}
		device->show_lacth(net);
		break;
		/*end------ADU*/
	...
	case IVL_LPM_MIP:
		if (device->show_mip == 0) {
			fprintf(stderr, "fpga.tgt: IVL_LPM_MIP not supported"
				" by this target.\n");
			return;
		}
		device->show_mip(net);
	...
	case IVL_LPM_SQUARE:
		if (device->show_square == 0) {
			fprintf(stderr, "fpga.tgt: IVL_LPM_SQUARE not supported"
				" by this target.\n");
			return;
		}
		device->show_square(net);
		break;

	case IVL_LPM_LMULT:
		if (device->show_lmult == 0) {
			fprintf(stderr, "fpga.tgt: IVL_LPM_LMULT not supported"
				" by this target.\n");
			return;
		}
		device->show_lmult(net);
		break;
	case IVL_LPM_LMULTOUT :
		if (device->show_lmultout == 0) {
			fprintf(stderr, "fpga.tgt: IVL_LPM_LMULTOUT not supported"
				" by this target.\n");
			return;
		}
		device->show_lmultout(net);
		break;

	case IVL_LPM_MULT_ADD:
		if (device->show_mult_add == 0) {
			fprintf(stderr, "fpga.tgt: IVL_LPM_MULT_ADD not supported"
				" by this target.\n");
			return;
		}
		device->show_mult_add(net);
		break;

	case IVL_LPM_LMULTADDOUT:
		if (device->show_mult_addout == 0) {
			fprintf(stderr, "fpga.tgt: IVL_LPM_MULT_ADD not supported"
				" by this target.\n");
			return;
		}
		device->show_mult_addout(net);
		break;

	case IVL_LPM_MULT_ACCUM:
		if (device->show_mult_accum == 0) {
			fprintf(stderr, "fpga.tgt: IVL_LPM_MULT_ACCUM not supported"
				" by this target.\n");
			return;
		}
		device->show_mult_accum(net);
		break;

	case IVL_LPM_HalfDSPBlock:
		if (device->show_halfdspblock == 0) {
			fprintf(stderr, "fpga.tgt: IVL_LPM_HalfDSPBlock not supported"
				" by this target.\n");
			return;
		}
		device->show_halfdspblock(net);
		break;
	...
	case IVL_LPM_RAM:
		if (device->show_ramDq == 0) {
			fprintf(stderr, "fpga.tgt: IVL_LPM_SHIFTR not supported"
				" by this target.\n");
			return;
		}
		device->show_ramDq(net);
		break;

	case IVL_LPM_DECODER:
		if (device->show_decoder == 0) {
			fprintf(stderr, "fpga.tgt: IVL_LPM_SHIFTR not supported"
				" by this target.\n");
			return;
		}
		device->show_decoder(net);
		break;

	  ...
	  case IVL_LPM_MULT_ADD:
		if (idx == 0) {
		  ...
		  return net->u_.mult.ena3;
			...
			assert(net->u_.mult.signa);
			return net->u_.mult.signa;
		}
		else if (idx == 13) {
			assert(net->u_.mult.signb);
			return net->u_.mult.signb;
		}
		else {
		  assert(0);
		  return 0;
...
/***********************/
extern "C" ivl_nexus_t ivl_lpm_multout_sbit(ivl_lpm_t net, unsigned idx)
{
	assert(net);
	switch (net->type) {
	case IVL_LPM_SQUARE:
	...
	case IVL_LPM_MULT_ADD:
	case IVL_LPM_LMULTADDOUT:
		if (idx == 0) {
			assert(net->u_.multout.clock0);
			return net->u_.multout.clock0;
		}
		else if (idx == 1) {
			assert(net->u_.multout.clock1);
			return net->u_.multout.clock1;
		}
		else if (idx == 2) {
			assert(net->u_.multout.clock2);
			return net->u_.multout.clock2;
		}
		else if (idx == 3) {
			assert(net->u_.multout.clock3);
			return net->u_.multout.clock3;
		}
		else if (idx == 4) {
			assert(net->u_.multout.aclr0);
			return net->u_.multout.aclr0;
		}
		else if (idx == 5) {
			assert(net->u_.multout.aclr1);
			return net->u_.multout.aclr1;
		}
		else if (idx == 6) {
			assert(net->u_.multout.aclr2);
			return net->u_.multout.aclr2;
		}
		else if (idx == 7) {
			assert(net->u_.multout.aclr3);
			return net->u_.multout.aclr3;
		}
		else if (idx == 8) {
			assert(net->u_.multout.ena0);
			return net->u_.multout.ena0;
		}
		else if (idx == 9) {
			assert(net->u_.multout.ena1);
			return net->u_.multout.ena1;
		}
		else if (idx == 10) {
			assert(net->u_.multout.ena2);
			return net->u_.multout.ena2;
		}
		else if (idx == 11) {
			assert(net->u_.multout.ena3);
			return net->u_.multout.ena3;
		}
		else if (idx == 12) {
			assert(net->u_.multout.signa);
			return net->u_.multout.signa;
		}
		else if (idx == 13) {
			assert(net->u_.multout.signb);
			return net->u_.multout.signb;
		}
		else {
			assert(0);
			return 0;
...
extern "C" ivl_nexus_t ivl_lpm_multout_dataa(ivl_lpm_t net, unsigned idx)
{
	assert(net);
	switch (net->type) {
	case IVL_LPM_MULT:
		...
		//case IVL_LPM_MULT_ADD:
		//case IVL_LPM_MULT_ACCUM:
	case IVL_LPM_LMULTOUT:
	case IVL_LPM_LMULTADDOUT:
		assert(idx < net->u_.multout.width_dataa);
		return net->u_.multout.dataa[idx];
	default:
		assert(0);
		return 0;
	}
...
extern "C" ivl_nexus_t ivl_lpm_multout_datab(ivl_lpm_t net, unsigned idx)
{
	assert(net);
	switch (net->type) {
	case IVL_LPM_MULT:
	...
	case IVL_LPM_LMULTOUT:
	case IVL_LPM_LMULTADDOUT:
		assert(idx < net->u_.multout.width_datab);
		return net->u_.multout.datab[idx];
	default:
		assert(0);
		return 0;
	}
...
extern "C" ivl_nexus_t ivl_lpm_multout_datac(ivl_lpm_t net, unsigned idx)
{
	assert(net);
	switch (net->type) {
	case IVL_LPM_MULT:
	...
	case IVL_LPM_LMULTOUT:
	case IVL_LPM_LMULTADDOUT:
		assert(idx < net->u_.multout.width_datac);
		return net->u_.multout.datac[idx];
	default:
		assert(0);
		return 0;
	}
...
extern "C" ivl_nexus_t ivl_lpm_multout_datad(ivl_lpm_t net, unsigned idx)
{
	assert(net);
	switch (net->type) {
	case IVL_LPM_MULT:
	...
	case IVL_LPM_LMULTOUT:
	case IVL_LPM_LMULTADDOUT:
		assert(idx < net->u_.multout.width_datad);
		return net->u_.multout.datad[idx];
	default:
		assert(0);
		return 0;
	}
...
extern "C" ivl_nexus_t ivl_lpm_multout_addsub(ivl_lpm_t net, unsigned idx)
{
	assert(net);
	switch (net->type) {
	case IVL_LPM_MULT:
	...
	case IVL_LPM_LMULTOUT:
	case IVL_LPM_LMULTADDOUT:
		assert(idx < net->u_.multout.width_addnsub);
		return net->u_.multout.addnsub[idx];
	default:
		assert(0);
		return 0;
	}
...
extern "C" ivl_nexus_t ivl_lpm_multout_accumsload(ivl_lpm_t net, unsigned idx)
{
	assert(net);
	switch (net->type) {
	case IVL_LPM_MULT:
	...
	case IVL_LPM_LMULTOUT:
	case IVL_LPM_LMULTADDOUT:
		assert(idx < net->u_.multout.width_accumsload);
		return net->u_.multout.accumsload[idx];
	default:
		assert(0);
		return 0;
	}
...
extern "C" ivl_nexus_t ivl_lpm_multout_result(ivl_lpm_t net, unsigned idx)
{
	assert(net);
	switch (net->type) {
	//case IVL_LPM_SQUARE:
	...
	case IVL_LPM_LMULTOUT:
	case IVL_LPM_LMULTADDOUT:
		assert(idx < net->u_.multout.width_result);
		return net->u_.multout.result[idx];
	default:
		assert(0);
		return 0;
	}
...
extern "C" unsigned ivl_lpm_multout_width(ivl_lpm_t net, unsigned idx)
{
	assert(net);
	switch (net->type) {
	case IVL_LPM_MULT:
	case IVL_LPM_LMULTOUT:
		if (idx == 0) return net->u_.multout.width_dataa;
		else if (idx == 1) return net->u_.multout.width_result;
	...
	case IVL_LPM_MULT_ACCUM:
		if (idx == 0) return net->u_.multout.width_dataa;
		//else if (idx == 1) return net->u_.multout.width_datab;
		//else if (idx == 2) return net->u_.mult.width_scanouta;
		//else if (idx == 3) return net->u_.mult.width_scanoutb;
		else if (idx == 4) return net->u_.multout.width_result;
		else {
			assert(0);
			return 0;
		};
	case IVL_LPM_LMULTADDOUT:		
		...
		else if (idx == 2) return net->u_.multout.width_datac;
		else if (idx == 3) return net->u_.multout.width_datad;
		else if (idx == 4) return net->u_.multout.width_addnsub;
		else if (idx == 5) return net->u_.multout.width_accumsload;
		else if (idx == 6) return net->u_.multout.width_result;
		else {
			assert(0);
			return 0;
		};
...
/***********************/
extern "C" ivl_nexus_t ivl_lpm_halfdspblock_dataa0(ivl_lpm_t net, unsigned idx)
{
	assert(net);
	switch (net->type) {
	case IVL_LPM_HalfDSPBlock:
		assert(idx < net->u_.halfdspblock.width_dataa0);
		return net->u_.halfdspblock.dataa0[idx];
	default:
		assert(0);
		return 0;
	}
}
extern "C" ivl_nexus_t ivl_lpm_halfdspblock_dataa1(ivl_lpm_t net, unsigned idx)
{
	assert(net);
	switch (net->type) {
	case IVL_LPM_HalfDSPBlock:
		assert(idx < net->u_.halfdspblock.width_dataa1);
		return net->u_.halfdspblock.dataa1[idx];
	default:
		assert(0);
		return 0;
	}
}
extern "C" ivl_nexus_t ivl_lpm_halfdspblock_datab0(ivl_lpm_t net, unsigned idx)
{
	assert(net);
	switch (net->type) {
	case IVL_LPM_HalfDSPBlock:
		assert(idx < net->u_.halfdspblock.width_datab0);
		return net->u_.halfdspblock.datab0[idx];
	default:
		assert(0);
		return 0;
	}
}
extern "C" ivl_nexus_t ivl_lpm_halfdspblock_datab1(ivl_lpm_t net, unsigned idx)
{
	assert(net);
	switch (net->type) {
	case IVL_LPM_HalfDSPBlock:
		assert(idx < net->u_.halfdspblock.width_datab1);
		return net->u_.halfdspblock.datab1[idx];
	default:
		assert(0);
		return 0;
	}
}
extern "C" ivl_nexus_t ivl_lpm_halfdspblock_result(ivl_lpm_t net, unsigned idx)
{
	assert(net);
	switch (net->type) {
	case IVL_LPM_HalfDSPBlock:
		assert(idx < net->u_.halfdspblock.width_result);
		return net->u_.halfdspblock.result[idx];
	default:
		assert(0);
		return 0;
	}
...
extern "C" ivl_nexus_t ivl_lpm_halfdspblock_clock(ivl_lpm_t net, unsigned idx)
{
	assert(net);
	switch (net->type) {
	case IVL_LPM_HalfDSPBlock:
		assert(idx < net->u_.halfdspblock.width_clock);
		return net->u_.halfdspblock.clock[idx];
	default:
		assert(0);
		return 0;
	}
}
extern "C" ivl_nexus_t ivl_lpm_halfdspblock_ena(ivl_lpm_t net, unsigned idx)
{
	assert(net);
	switch (net->type) {
	case IVL_LPM_HalfDSPBlock:
		assert(idx < net->u_.halfdspblock.width_ena);
		return net->u_.halfdspblock.ena[idx];
	default:
		assert(0);
		return 0;
	}
}
extern "C" ivl_nexus_t ivl_lpm_halfdspblock_accumsload(ivl_lpm_t net, unsigned idx)
{
	assert(net);
	switch (net->type) {
	case IVL_LPM_HalfDSPBlock:
		assert(idx < net->u_.halfdspblock.width_accumsload);
		return net->u_.halfdspblock.accumsload[idx];
	default:
		assert(0);
		return 0;
	}
...
extern "C" unsigned ivl_lpm_halfdspblock_width(ivl_lpm_t net, unsigned idx)
{
	assert(net);
	switch (net->type) {
	case IVL_LPM_MULT:
	...
	case IVL_LPM_HalfDSPBlock:		
		if        (idx == 0) return net->u_.halfdspblock.width_dataa0;
		else if (idx == 1) return net->u_.halfdspblock.width_datab0;
		else if (idx == 2) return net->u_.halfdspblock.width_dataa1;
		else if (idx == 3) return net->u_.halfdspblock.width_datab1;
		else if (idx == 4) return net->u_.halfdspblock.width_result;
		else if (idx == 5) return net->u_.halfdspblock.width_clock;
		else if (idx == 6) return net->u_.halfdspblock.width_ena;
		else if (idx == 7) return net->u_.halfdspblock.width_accumsload;
		else {
			assert(0);
			return 0;
		};
...
//decoder
extern "C" unsigned ivl_lpm_decoder_width(ivl_lpm_t net){
	assert(net);
	return net->u_.decoder.width;
}

extern "C" unsigned ivl_lpm_decoder_decodes(ivl_lpm_t net){
	assert(net);
	return net->u_.decoder.decodes;
}

extern "C" ivl_nexus_t ivl_lpm_decoder_data(ivl_lpm_t net,int idx){
	assert(idx < net->u_.decoder.width);
	return net->u_.decoder.data[idx];
}

extern "C" ivl_nexus_t ivl_lpm_decoder_eq(ivl_lpm_t net,int idx){
	assert(idx < net->u_.decoder.decodes);
	return net->u_.decoder.eq[idx];
}

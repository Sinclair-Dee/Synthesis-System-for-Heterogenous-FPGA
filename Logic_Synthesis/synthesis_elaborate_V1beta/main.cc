...
#include  "flag.h"
flat_mult *temp;
flat_mult *Flag_mult = initflag(temp);
...
#ifdef __VERIFICATION__
list<perm_string> clock_names;
#endif 
...
#ifdef __VERIFICATION__
static void print_clock_name(){
	//print clock for Xuyu
	assert(roots.size() >= 1);
	const char* root_odule_name = roots.front().str();//Here we assume that there is only one root module
	string file_name = base;
	file_name += "\\";
	file_name += root_odule_name;
	file_name += ".clk";
	ofstream clk_file(file_name.c_str());
	list<perm_string>::iterator idx ;
	for (idx = clock_names.begin(); idx != clock_names.end(); idx++)
	{
		clk_file << idx->str();
		clk_file << " ";
	}
...
#          endif
	...
	if(argc >= 4){
		cout << "project path: " << argv[1] << "." <<endl;
		cout << "project name: " << argv[2] << "." <<endl;
		cout << "specified top module: " << argv[3] << "." <<endl;
		roots.push_back(argv[3]);
	}else if(argc == 3){
		cout << "project path: " << argv[1] << "." <<endl;
		cout << "project name: " << argv[2] << "." <<endl;
		roots.push_back(argv[2]);
	}else{
		cerr << "error: The correct command parameter is: work_path project_name [specified_top_module]." << endl;
		return 1;
	}
	...
	strcat(user_in_file_path_name,".v");//modify at 20170315
	...
	/* Count IP cores here!!! -- CCC */
	cout << "COUNT IP CORES" << endl;
	map<perm_string,Module*>::iterator modcount;
	for (modcount = pform_modules.begin(); 
	  	  modcount != pform_modules.end(); 
	   	modcount++) {
			//获取gate
			list<PGate*> gates = (*modcount).second->get_gates();
			list<PGate*>::const_iterator gate;

			//test 统计例化ip核的种类
			static unsigned num_of_dsp =  0;
			static unsigned num_of_mem = 0;

			for (gate = gates.begin(); gate != gates.end(); gate++) {
				PGModule *modcount = dynamic_cast<PGModule*>(*gate);
				if (modcount) {
					if (strcmp(modcount->get_type(),"altmult_add") == 0) {
						num_of_dsp++;
						cout << "  DSP core(s): altmult_accum->" <<num_of_dsp<< endl;
					}
					if (strcmp(modcount->get_type(),"altsyncram") == 0) {
						num_of_mem++;
						cout << "  Memory core(s): altsyncram->" <<num_of_mem <<endl;
					}
				}
			}
	}
	...
	for (mod = pform_modules.begin(); 
		...
		static bool mult_add_parse_flag = false;
		static bool mult_accum_parse_flag = false;

		static bool mult_add_parse_flag_used = false;
		static bool mult_accum_parse_flag_used = false;

		//获取mult中参数defparms
			map<hname_t,PExpr*>::iterator defparm;
			for (defparm = (*mod).second->defparms.begin(); 
				defparm != (*mod).second->defparms.end(); 
				defparm++) {
					PExpr *temp_parm = defparm->second;
						PEString *parm= dynamic_cast<PEString*>(temp_parm);
						if (parm){
							string ptext = parm->value();				
							if (ptext =="altmult_accum")
								mult_accum_parse_flag = true;
							else if (ptext == "altmult_add")
								mult_add_parse_flag = true;
						}	
			}

			for (gate = gates.begin(); (!mult_add_parse_flag_used) && (!mult_accum_parse_flag_used) && gate != gates.end(); gate++) {
				PGModule *modle = dynamic_cast<PGModule*>(*gate);
				if (modle) {
					if (strcmp(modle->get_type(),"altmult_add") == 0) {
						cout << "  DSP core(s): altmult_add" << endl;
						if (mult_add_parse_flag) {
							mult_add_parse_flag_used = true;
							rc = pform_parse("altmult_add.v");
						} else if (mult_accum_parse_flag) {
							mult_accum_parse_flag_used = true;
							rc = pform_parse("altmult_laccum.v");
						}
			...
			for (gate = gates.begin(); gate != gates.end(); gate++) {
				...
				if (mod) {
					if (strcmp(mod->get_type(),"altmult_accum") == 0) {
						...
						rc = pform_parse("altmult_accum.v");

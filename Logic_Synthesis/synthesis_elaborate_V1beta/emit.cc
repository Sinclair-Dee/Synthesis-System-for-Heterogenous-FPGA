...
bool NetDecoder::emit_node(struct target_t*tgt) const
{
	tgt->lpm_decoder(this);
	return true;
}
...
bool NetMem::emit_node(struct target_t*tgt) const
{
	tgt->lpm_mem(this);
	return true;
}

bool NetSquare::emit_node(struct target_t*tgt) const
{
	tgt->lpm_square(this);
	return true;
}

bool NetLMult::emit_node(struct target_t*tgt) const
{
	tgt->lpm_lmult(this);
	return true;
}
bool NetLMultSimpleOUT::emit_node(struct target_t*tgt) const
{
	tgt->lpm_lmultout(this);
	return true;
}

bool NetMultAdd::emit_node(struct target_t*tgt) const
{
	tgt->lpm_mult_add(this);
	return true;
}

bool NetMultAddOut::emit_node(struct target_t*tgt) const
{
	tgt->lpm_mult_addout(this);
	return true;
}

bool NetMultAccum::emit_node(struct target_t*tgt) const
{
	tgt->lpm_mult_accum(this);
	return true;
}
bool  HalfDSPBlock::emit_node(struct target_t*tgt) const
{
	tgt->lpm_HalfDSPBlock(this);
	return true;
}
	...
	if (signals_) {
		...
		do {
			...
			for(unsigned idx = 0; idx < cur->pin_count(); idx++){
				if(cur->pin(idx).nexus()->is_driven() >=2)//É÷ÖØ£¡£¡£¡
					assert(0);
			}

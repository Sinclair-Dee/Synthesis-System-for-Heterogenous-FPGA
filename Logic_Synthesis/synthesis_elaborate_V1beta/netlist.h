...
public:
	...
	//virtual const NetScope* top_scope()const;//ADU:
...
public:
	...
	//添加函数——ADU
	bool Link::is_linkedToSignificantSignal() const;
...
public:
	...
	//ADU:
	bool at_least_one_pin_linked_to_signi_signal()const;
...
//ADU
class NetDecoder  : public NetNode {

public:
	NetDecoder(NetScope*s, perm_string name, unsigned wid, unsigned decodes);
	virtual ~NetDecoder(){}
	...
	Link& pin_Eq(unsigned idx);
	const Link& pin_Eq(unsigned idx) const;
...
public:
	...
	bool set_mem(NetMemory *mem);//ADU
	NetMemory* get_mem();//ADU
...
public:
	...
	//ADU
	NetRamDq *get_ramlist();
	NetMemoryRef* get_ram_ref(){return mem_ref_;}
	void set_mem_ref(NetMemoryRef *mem_ref){mem_ref_ = mem_ref;}
...
public:
	...
	NetMemoryRef(NetMemory* mem, NetNet* waddr, NetNet* datain, NetNet* clk, NetNet* we)
		:mem_(mem),waddr_(waddr),datain_(datain),clk_(clk),we_(we){}
	~NetMemoryRef(){}

	NetMemory* get_mem(){return mem_;}
	NetNet* get_waddr(){return waddr_;}
	NetNet* get_datain(){return datain_;}
	NetNet* get_clk(){return clk_;}
	NetNet* get_we(){return we_;}

	void set_mem( NetMemory* mem){mem_ = mem;}
	void set_waddr( NetNet* waddr){waddr_ = waddr;}
	void set_datain( NetNet* datain){datain_ = datain;}
	void set_clk( NetNet* clk){clk_ = clk;}
	void set_we( NetNet* we){we_ = we;}
private:
	NetMemory* mem_;
	NetNet* waddr_;
	NetNet* datain_;
...
class NetMem  : public NetNode {

public:
	NetMem(NetScope*sc, perm_string n,
	...
	// Get the width of the device bussed inputs. There are these
	// parameterized widths:
	unsigned width_address_a() const;
	unsigned width_address_b() const;
	unsigned width_data_a() const;
	unsigned width_data_b() const;
	unsigned width_q_a() const;
	unsigned width_q_b() const;
	...
	Link& pin_address_a(unsigned idx);
	Link& pin_address_b(unsigned idx);
	Link& pin_data_a(unsigned idx);
	Link& pin_data_b(unsigned idx);
	Link& pin_q_a(unsigned idx);
	Link& pin_q_b(unsigned idx);

	const Link& pin_clocken0() const;
	const Link& pin_clocken1() const;
	const Link& pin_wren_a() const;
	const Link& pin_aclr0() const;
	const Link& pin_clock0() const;
	const Link& pin_wren_b() const;
	const Link& pin_aclr1() const;
	const Link& pin_clock1() const;

	const Link& pin_address_a(unsigned idx) const;
	const Link& pin_address_b(unsigned idx) const;
	const Link& pin_data_a(unsigned idx) const;
	const Link& pin_data_b(unsigned idx) const;
	const Link& pin_q_a(unsigned idx) const;
	const Link& pin_q_b(unsigned idx) const;
...
private:
	unsigned width_address_a_;
	unsigned width_address_b_;
	unsigned width_data_a_;
	unsigned width_data_b_;
	unsigned width_q_a_;
	unsigned width_q_b_;
};

class NetSquare  : public NetNode {

public:
	NetSquare(NetScope*sc, perm_string n,
		unsigned w_data, unsigned w_result);
	~NetSquare();

	// Get the width of the device bussed inputs. There are these
	// parameterized widths:
	unsigned width_data() const;
	unsigned width_result() const;
...
private:
	unsigned width_data_;
	unsigned width_result_;
};

class NetLMult  : public NetNode {

public:
	NetLMult(NetScope*sc, perm_string n,
		unsigned w_dataa, unsigned w_datab, unsigned w_sum, unsigned w_result);
	~NetLMult();

	// Get the width of the device bussed inputs. There are these
	// parameterized widths:
	unsigned width_dataa() const;
	unsigned width_datab() const;
	unsigned width_sum() const;
	unsigned width_result() const;
...
private:
	unsigned width_dataa_;
	unsigned width_datab_;
	unsigned width_sum_;
	unsigned width_result_;
};

class NetLMultSimpleOUT  : public NetNode {

public:
	NetLMultSimpleOUT(NetScope*sc, perm_string n,
		unsigned w_dataa, unsigned w_result);
	~NetLMultSimpleOUT();
...
class NetMultAddOut : public NetNode{

public:
	NetMultAddOut(NetScope*sc, perm_string n,
	...
	unsigned width_dataa() const;
	unsigned width_datab() const;
	unsigned width_datac() const;
	unsigned width_datad() const;
	unsigned width_addnsub() const;
	unsigned width_accumsload() const;
	unsigned width_result() const;
	...
	Link& pin_dataa(unsigned idx);
	Link& pin_datab(unsigned idx);
	Link& pin_datac(unsigned idx);
	Link& pin_datad(unsigned idx);
	Link& pin_addnsub(unsigned idx);
	Link& pin_accumsload(unsigned idx);
	Link& pin_result(unsigned idx);

	const Link& pin_clock0() const;
	const Link& pin_clock1() const;
	const Link& pin_clock2() const;
	const Link& pin_clock3() const;
	const Link& pin_aclr0() const;
	const Link& pin_aclr1() const;
	const Link& pin_aclr2() const;
	const Link& pin_aclr3() const;
	const Link& pin_ena0() const;
	const Link& pin_ena1() const;
	const Link& pin_ena2() const;
	const Link& pin_ena3() const;
	const Link& pin_signa() const;
	const Link& pin_signb() const;

	const Link& pin_dataa(unsigned idx) const;
	const Link& pin_datab(unsigned idx) const;
	const Link& pin_datac(unsigned idx) const;
	const Link& pin_datad(unsigned idx) const;
	const Link& pin_addnsub(unsigned idx) const;
	const Link& pin_accumsload(unsigned idx) const;
	const Link& pin_result(unsigned idx) const;
...
private:
	unsigned width_dataa_;
	unsigned width_datab_;
	unsigned width_datac_;
	unsigned width_datad_;
	unsigned width_addnsub_;
	unsigned width_accumsload_;
	unsigned width_result_;
};

class NetMultAdd : public NetNode {

public:
	NetMultAdd(NetScope*sc, perm_string n,
	...
	unsigned width_dataa() const;
	unsigned width_datab() const;
	unsigned width_scanouta() const;
	unsigned width_scanoutb() const;
	unsigned width_result() const;
	...
	Link& pin_dataa(unsigned idx);
	Link& pin_datab(unsigned idx);
	Link& pin_scanouta(unsigned idx);
	Link& pin_scanoutb(unsigned idx);
	Link& pin_result(unsigned idx);
	...
	const Link& pin_dataa(unsigned idx) const;
	const Link& pin_datab(unsigned idx) const;
	const Link& pin_scanouta(unsigned idx) const;
	const Link& pin_scanoutb(unsigned idx) const;
	const Link& pin_result(unsigned idx) const;
...
private:
	unsigned width_dataa_;
	unsigned width_datab_;
	unsigned width_scanouta_;
	unsigned width_scanoutb_;
	unsigned width_result_;
};

//class NetLMultADDOUT0 : public NetLMultSimpleOUT{
//public:
//	NetLMultADDOUT0(NetScope*sc, perm_string n,
//		unsigned w_dataa, unsigned w_datab, unsigned w_scanouta, unsigned w_scanoutb, unsigned w_result)
//	~NetLMultADDOUT0();
//
//	 Get the width of the device bussed inputs. There are these
...
class NetMultAccum  : public NetNode {

public:
	NetMultAccum(NetScope*sc, perm_string n,
		unsigned w_dataa, unsigned w_datab, unsigned w_scanouta, unsigned w_scanoutb, unsigned w_result);
	~NetMultAccum();
	...
	const Link& pin_accum_sload() const;
	const Link& pin_clock0() const;
	const Link& pin_clock1() const;
	const Link& pin_clock2() const;
	const Link& pin_clock3() const;
...
HalfDSPBlock
 ************************/
class HalfDSPBlock  : public NetNode {

public:
HalfDSPBlock(NetScope*sc, perm_string n,
						//data
...
// Get the width of the device bussed inputs. There are these
// parameterized widths:
unsigned width_dataa0() const;
unsigned width_datab0() const;
unsigned width_dataa1() const;
unsigned width_datab1() const;
unsigned width_dataa2() const;
unsigned width_datab2() const;
unsigned width_dataa3() const;
unsigned width_datab3() const;

unsigned width_scanina() const;
unsigned width_scanouta() const;

unsigned width_chainin() const;
unsigned width_chainout() const;

unsigned width_clock() const;
unsigned width_ena() const;
unsigned width_aclr() const;

//control
unsigned width_zeroloopback() const;
unsigned width_accumsload() const;
unsigned width_zerochainout()  const;
unsigned width_chainoutround() const;
unsigned width_chainsaturate() const;
unsigned width_signa() const;
unsigned width_signb()  const;
unsigned width_outputround() const;
unsigned width_outsaturate() const;;
unsigned width_rotate() const;
unsigned width_shiftright() const;

unsigned width_OverFlow() const;
unsigned width_ChainOutSatOverFlow() const;

unsigned width_result() const;

Link& pin_dataa0(unsigned idx);
Link& pin_datab0(unsigned idx);
Link& pin_dataa1(unsigned idx);
Link& pin_datab1(unsigned idx);
Link& pin_dataa2(unsigned idx);
Link& pin_datab2(unsigned idx);
Link& pin_dataa3(unsigned idx);
Link& pin_datab3(unsigned idx);

Link& pin_scanina(unsigned idx);
Link& pin_scanoua(unsigned idx);

Link& pin_clock(unsigned idx);
Link& pin_ena(unsigned idx);
Link& pin_aclr(unsigned idx);

Link& pin_chainin(unsigned idx);
Link& pin_chainout(unsigned idx);
Link& pin_zeroloopback(unsigned idx);
Link& pin_accumsload(unsigned idx);
Link& pin_zerochainout(unsigned idx);
Link& pin_chainoutround(unsigned idx);
Link& pin_chainsaturate(unsigned idx);
Link& pin_signa(unsigned idx);
Link& pin_signb(unsigned idx);
Link& pin_outputround(unsigned idx);
Link& pin_outsaturate(unsigned idx);
Link& pin_rotate(unsigned idx);
Link& pin_shiftright(unsigned idx);

Link& pin_OverFlow(unsigned idx);
Link& pin_ChainOutSatOverFlow(unsigned idx);

Link& pin_result(unsigned idx);


const Link& pin_dataa0(unsigned idx) const;
const Link& pin_datab0(unsigned idx) const;
const Link& pin_dataa1(unsigned idx) const;
const Link& pin_datab1(unsigned idx) const;
const Link& pin_dataa2(unsigned idx) const;
const Link& pin_datab2(unsigned idx) const;
const Link& pin_dataa3(unsigned idx) const;
const Link& pin_datab3(unsigned idx) const;

const Link& pin_scanina(unsigned idx) const;
const Link& pin_scanoua(unsigned idx) const;

const Link& pin_chainin(unsigned idx) const;
const Link& pin_chainout(unsigned idx) const;

const Link& pin_clock(unsigned idx) const;
const Link& pin_ena(unsigned idx) const;
const Link& pin_aclr(unsigned idx) const;

const Link& pin_zeroloopback(unsigned idx) const;
const Link& pin_accumsload(unsigned idx) const;
const Link& pin_zerochainout(unsigned idx) const;
const Link& pin_chainoutround(unsigned idx) const;
const Link& pin_chainsaturate(unsigned idx) const;
const Link& pin_signa(unsigned idx) const;
const Link& pin_signb(unsigned idx) const;
const Link& pin_outputround(unsigned idx) const;
const Link& pin_outsaturate(unsigned idx) const;
const Link& pin_rotate(unsigned idx) const;
const Link& pin_shiftright(unsigned idx) const;

const Link& pin_OverFlow(unsigned idx) const;
const Link& pin_ChainOutSatOverFlow(unsigned idx) const;
...
private:
unsigned width_dataa0_;
unsigned width_datab0_;
unsigned width_dataa1_;
unsigned width_datab1_;
unsigned width_dataa2_;
unsigned width_datab2_;
unsigned width_dataa3_;
unsigned width_datab3_;

unsigned width_scanina_;
unsigned width_scanouta_;
unsigned width_chainin_;
unsigned width_chainout_;
unsigned width_clock_;
unsigned width_ena_;
unsigned width_aclr_;
unsigned width_zeroloopback_;
unsigned width_accumsload_;
unsigned width_zerochainout_;
unsigned width_chainoutround_;
unsigned width_chainsaturate_;
unsigned width_signa_;
unsigned width_signb_;
unsigned width_outputround_;
unsigned width_outsaturate_;
unsigned width_rotate_;
unsigned width_shiftright_;

unsigned width_OverFlow_;
unsigned width_ChainOutSatOverFlow_;
unsigned width_result_;
};
...
public:
	...
	Link& pin_RAddress(unsigned idx);
	Link& pin_WAddress(unsigned idx);
	Link& pin_Data(unsigned idx);
	Link& pin_Q(unsigned idx);
	...
	const Link& pin_RAddress(unsigned idx) const;
	const Link& pin_WAddress(unsigned idx) const;
	const Link& pin_Data(unsigned idx) const;
	const Link& pin_Q(unsigned idx) const;
	...
	// connections.
	NetRamDq* get_next_ramDq(){return next_;}
	//void absorb_partners();//暂时注释
...
public:
	...
	virtual NexusSet* nex_input(bool rem_out = false) =0;//ADU
...
public:
	...
	// sensitivity list.
	virtual NexusSet* nex_input(bool rem_out = false);//ADU This make the sense!!
...
//#endif
	...
	bool synth_async_accum(Design*des, NetScope*scope, bool sync_flag,
		struct sync_accounting_cell*nex_ff,
...
public:
	...
	virtual NexusSet* nex_input(bool rem_out = false);//ADU
	virtual void nex_output(NexusSet&);
	virtual bool emit_proc(struct target_t*) const;
	virtual int match_proc(struct proc_match_t*);
	virtual void dump(ostream&, unsigned ind) const;
...
private:
	...
	int connect_enable(Design*des, NetScope*scope,
		NetFF* ff, NetNet*ce);
private:
	NetExpr* expr_;
	NetProc*if_;

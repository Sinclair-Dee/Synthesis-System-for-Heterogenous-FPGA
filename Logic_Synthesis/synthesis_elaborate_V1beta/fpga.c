...
int target_design(ivl_design_t des)
{
      ...
      const char* pathNameTail= ivl_design_flag(des, "-o");

	  strcat(user_out_file_path_name,base);
	  strcat(user_out_file_path_name, "\\");
	  strcat(user_out_file_path_name, pathNameTail);

      xnf = fopen(user_out_file_path_name, "w");
      if (xnf == 0) {
	    perror(user_out_file_path_name);
	    return -1;
      }

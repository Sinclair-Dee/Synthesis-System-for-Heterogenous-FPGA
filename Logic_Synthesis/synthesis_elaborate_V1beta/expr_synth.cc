...
{
      //NetEConst*lcon = reinterpret_cast<NetEConst*>(left_);//---ADU
      //NetEConst*rcon = reinterpret_cast<NetEConst*>(right_);
      NetEConst*lcon = dynamic_cast<NetEConst*>(left_);
      NetEConst*rcon = dynamic_cast<NetEConst*>(right_);
  ...
  //    if ((rcon && (rcon->value() == verinum(0UL,rcon->expr_width())))
	 ...
	 //   switch (op_) {
		...
		//	//assert(0);
		//	cout << get_line()
		//       << ": warning: 0 > " << lsig->name() << " shouldn't happen." << endl;
		//	gate = new NetLogic(scope, scope->local_symbol(),
		//		      lsig->pin_count()+1, NetLogic::NOR);
		//  }
		//  break;
		...
		//	//assert(0);
		//	cout << get_line()
		//       << ": warning: " << lsig->name() << " < 0 " << " shouldn't happen." << endl;
		//	gate = new NetLogic(scope, scope->local_symbol(),
		//		      lsig->pin_count()+1, NetLogic::NOR);
		//  }
		//  break;

...
PMemAssign::PMemAssign(svector<PExpr*>*pins)
: PGate(perm_string(), pins)
{
      assert(pins->count() == 16);
}

PMemAssign::~PMemAssign()
{
}

PSquareAssign::PSquareAssign(svector<PExpr*>*pins)
: PGate(perm_string(), pins)
{
      assert(pins->count() == 5);
}

PSquareAssign::~PSquareAssign()
{
}

PMultAssign::PMultAssign(svector<PExpr*>*pins)
: PGate(perm_string(), pins)
{
      assert(pins->count() == 7);
}

PMultAssign::~PMultAssign()
{
}

PMultAddAssign::PMultAddAssign(svector<PExpr*>*pins)
: PGate(perm_string(), pins)
{
      assert(pins->count() == 19);
}

PMultAddAssign::~PMultAddAssign()
{
}

PMultAccumAssign::PMultAccumAssign(svector<PExpr*>*pins)
: PGate(perm_string(), pins)
{
      assert(pins->count() == Mult_Acc_Parse_Number);
}

PMultAccumAssign::~PMultAccumAssign()
{
}

...
void blif_print(FILE*fd, edif_t edf)
{
	...
	char celldata_txt_path[1024] = {'\0'};
	FILE* pll_info = 0;
	FILE* m4k_info = 0;
	FILE*celldata_info = 0;
	strcat(pll_info_path, base);
	strcat(pll_info_path,"\\comet_pll.info");
	strcat(celldata_txt_path,base);
	strcat(celldata_txt_path,"\\celldata_info.txt");
	strcat(m4k_info_path, base);
	strcat(m4k_info_path,"\\comet_m4k.info");


	//int tempa;
	//tempa= 1;/*Flag.getflag_mult_signed_dataa0();
	...
	celldata_info = fopen(celldata_txt_path, "w");
	if (!celldata_info)
	{
		printf("error: cannot open the file %s", celldata_txt_path);
		exit(1);
	}
	...
	for (xlib = edf->xlibs; xlib; xlib = xlib->next) {
		for (cell = xlib->cells; cell; cell = cell->next) {
			//fprintf(celldata_info," %s",cell->name);
			//fflush(celldata_info);
			int done = 0;
			for (ref = edf->celref; ref; ref = ref->next) {
				//fprintf(celldata_info," %s", ref->cell->name);
				//fflush(celldata_info);
	...
	/* the root module *///modified in 2017.05.31
	fprintf(fd, ".model %s\n", edf->name);
	...
	fprintf(fd, "\n");
	fflush(fd);  //all input port of root module!
	...
	fprintf(fd, "\n");
	fflush(fd);  //all output port of root module!
	...
	for (ref = edf->celref; ref; ref = ref->next) {
		...
		}/*else if(strncmp(ref->cell->name, "mult_",5)== 0){
		...
		}else if(strcmp(ref->cell->name, "cell0")== 0){

		 }else if(strcmp(ref->cell->name, "cell1")== 0){

		 }*/

		fprintf(fd, ".subckt %s", ref->cell->name);
		//test 
		if (strcmp(ref->cell->name,"dual_port_ram")){
			unsigned stopstopstop = 1;
		}
		idx = 0;
		for (jnt = edf->nexa; jnt; jnt = jnt->next, idx += 1) {
			...
			for (jc = jnt->links; jc; jc = jc->next)
				if (jc->cell && jc->cell->u == ref->u)
					if (net_name[idx]){
						...
						//if (strncmp(ref->cell->name, "mult_18",7) == 0
						//	&&(strncmp(jc->cell->cell->ports[jc->port].name, "dataa_",6) == 0
						//	||strncmp(jc->cell->cell->ports[jc->port].name, "datab_",6) == 0)){
						//		fprintf(fd, " %s=N0", jc->cell->cell->ports[jc->port].name/*, 0*/);
						//}
						////else if(strncmp(ref->cell->name, "mult_1add_out",13)==0 &&
						////			strcmp(jc->cell->cell->ports[jc->port].name,"clock0")!=0)
						////				fprintf(fd, " clock0=%s", "clock0");
						//else{
							fprintf(fd, " %s=N%u", jc->cell->cell->ports[jc->port].name, idx);
						 	fflush(fd);
					}
...
void blif_show_cell(FILE*fd, edif_cell_t cell)
{
	...
	/*END--ADU*/
		...
		fprintf(fd, ".names Data0 Result0\n");//modified on 2017.4.11 
		fprintf(fd, "1 1\n");
	}
	...
	//else if (name[0] == 'm' && name[1] == 'u' && name[2] == 'l' && name[3] == 't' && name[4] != '_' && type == "LPM_MULTOUT") {
	//	unsigned width_p = 0;
	//	unsigned width_a = 0;
	//	for (prp = cell->property; prp; prp = prp->next) {
	//		if (prp->name == "LPMOUT_Width_Data_A")
	//			width_a = prp->value_.num;
	//		if (prp->name == "LPMOUT_Width_Result")
	//			width_p = prp->value_.num;
	//	}
	//	//assert(width_p);
	//	//assert(width_a);
	...
	//}
	...
	else if (name[0] == 'd' && name[1] == 'u' && name[2] == 'a' && name[3] == 'l'&& name[4] == '_'
		&& name[5] == 'p' && name[6] == 'o' &&  name[7] == 'r'&& type == "LPM_MIP") {
		fprintf(fd, "#");
		for (prp = cell->property; prp; prp = prp->next) {
			if (prp->name == "LPM_Width_Address_A")
				fprintf(fd, " Width_Address_A = %i", prp->value_.num);
	...
	else if (/*name[0] == 'm' && name[1] == 'u' && name[2] == 'l' && name[3] == 't' && name[4] == '_' &&*/ type == "NetLMultSimpleOUT") {
		fprintf(fd, "# operation_mode = \"output_only\"\n");
		for (prp = cell->property; prp; prp = prp->next) {
			if (prp->name == "LPMOUT_Width_Data_A")
				fprintf(fd, "# dataa_width = %d\n", prp->value_.num);
			else if (prp->name == "LPMOUT_Width_Result")
			fprintf(fd, "# result_width = %d\n", prp->value_.num);
		}
		fprintf(fd, "# addnsub0_clock = \"none\"\n");
		fprintf(fd, "# addnsub1_clock = \"none\"\n");
		fprintf(fd, "# zeroacc_clock = \"none\"\n");
		if(Flag_mult->flag_mult_signed_dataa0 )
				fprintf(fd, "# signa_clock = \"gnd\"\n");
		else
				fprintf(fd, "# signa_clock = \"none\"\n");
		if(Flag_mult->flag_mult_signed_datab0 )
				fprintf(fd, "# signb_clock = \"gnd\"\n");
		else
				fprintf(fd, "# signb_clock = \"none\"\n");
		fprintf(fd, "# output_clock = \"none\"\n");
		fprintf(fd, "# addnsub0_pipeline_clock = \"none\"\n");
		fprintf(fd, "# addnsub1_pipeline_clock = \"none\"\n");
		fprintf(fd, "# zeroacc_pipeline_clock = \"none\"\n");
		fprintf(fd, "# signa_pipeline_clock = \"none\"\n");
		fprintf(fd, "# signb_pipeline_clock =  \"none\"\n");
		fprintf(fd, "# addnsub0_clear = none\n");
		fprintf(fd, "# addnsub1_clear = none\n");
		fprintf(fd, "# zeroacc_clear = \"none\"\n");
		fprintf(fd, "# signa_clear = \"none\"\n");
		fprintf(fd, "# signb_clear = \"none\"\n");
		fprintf(fd, "# output_clear = \"none\"\n");
		fprintf(fd, "# addnsub0_pipeline_clear = \"none\"\n");
		fprintf(fd, "# addnsub1_pipeline_clear = \"none\"\n");
		fprintf(fd, "# zeroacc_pipeline_clear = \"none\"\n");
		fprintf(fd, "# signa_pipeline_clear = \"none\"\n");
		fprintf(fd, "# signb_pipeline_clear = \"none\"\n");
		//fprintf (fd, "\n");
		//fprintf(fd, "# dataa \n");
		//fprintf(fd, "# result\n");
		fprintf(fd, ".blackbox\n");
		...
		for (prp = cell->property; prp; prp = prp->next) {
			if (prp->name == "LPM_Width_Data_A")
				...
				fprintf(fd, " # datab_width = %d\n", prp->value_.num);
			//else if (prp->name == "LPM_Width_Sum\n")
			//	fprintf(fd, " Width_Sum = %d", prp->value_.num);
			else if (prp->name == "LPM_Width_Result\n")
				fprintf(fd, " # result_width = %d", prp->value_.num);
		}
		//fprintf (fd, "\n");
		fprintf(fd, "# dataa_clock = \"none\"\n");
		fprintf(fd, "# datab_clock = \"none\"\n");
		if(Flag_mult->flag_mult_signed_dataa0 )
			fprintf(fd, "# signa_clock = \"gnd\"\n");
		else
			fprintf(fd, "# signa_clock = \"none\"\n");
		if(Flag_mult->flag_mult_signed_dataa0 )
			...
			fprintf(fd, "# signb_clock = \"none\"\n");
		fprintf(fd, "# output_clock = \"none\"\n");
		fprintf(fd, "# dataa_clear = \"none\"\n");
		fprintf(fd, "# datab_clear = \"none\"\n");
		fprintf(fd, "# signa_clear = \"none\"\n");
		fprintf(fd, "# signb_clear = \"none\"\n");
		fprintf(fd, "# output_clear = \"none\"\n");
		fprintf(fd, "# signa_internally_grounded = \"none\"\n");
		fprintf(fd, "# signb_internally_grounded = \"none\"\n");

		fprintf(fd, ".blackbox\n");
	}
	else if (name[0] == 'm' && name[1] == 'u' && name[2] == 'l' && name[3] == 't' && name[4] == 'i' 
		 &&name[5] == 'p' && name[6] == 'l' && name[7] == 'y'   && type == "LPM_HalfDSPBlock") {
			 for (prp = cell->property; prp; prp = prp->next) {
				 if (prp->name == "LPM_Width_Data_A0")
					 fprintf(fd, "# dataa_width = %d\n", prp->value_.num);
				 else if (prp->name == "LPM_Width_Data_B0")
					 fprintf(fd, "# datab_width = %d\n", prp->value_.num);
			 }
			 fprintf(fd, ".blackbox\n");
	}
		...
		for (prp = cell->property; prp; prp = prp->next) {
			...
			//	fprintf(fd, " Width_Scanout_A = %d", prp->value_.num);
			//else if (prp->name == "LPM_Width_Scanout_B")
			//	fprintf(fd, " Width_Scanout_B = %d", prp->value_.num);
			//else if (prp->name == "LPM_Width_Result")
			//	fprintf(fd, " Width_Result = %d", prp->value_.num);
		}
		fprintf(fd, "# addnsub0_clock = \"none\"\n");
		fprintf(fd, "# addnsub1_clock = \"none\"\n");
		fprintf(fd, "# zeroacc_clock = \"none\"\n");
		...
		else if (type == "LPM_MULT_ADD_OUT") {
			for (prp = cell->property; prp; prp = prp->next) {
				if (prp->name == "LPM_Width_Data_A")
					fprintf(fd, "#dataa_width = %d\n", prp->value_.num);
				else if (prp->name == "LPM_Width_Data_B")
					fprintf(fd, "#datab_width = %d\n", prp->value_.num);
				else if (prp->name == "LPM_Width_Data_C")
					fprintf(fd, "#datac_width = %d\n", prp->value_.num);
				else if (prp->name == "LPM_Width_Data_D")
					fprintf(fd, "#datad_width = %d\n", prp->value_.num);
				//else if (prp->name == "LPM_Width_Result")
				//	fprintf(fd, " #Width_Result = %d\n", prp->value_.num);
			}
			//fprintf (fd, "\n");
			fprintf (fd, "#operation_mode = \"two_level_adder\"\n");
			fprintf(fd, "# addnsub0_clock = \"none\"\n");
			fprintf(fd, "# addnsub1_clock = \"none\"\n");
			fprintf(fd, "# zeroacc_clock = \"none\"\n");
			fprintf(fd, "# signa_clock = \"none\"\n");
		...
		for (prp = cell->property; prp; prp = prp->next) {
			if (prp->name == "LPM_Width_Data_A")
				fprintf(fd, " Width_Data_A = %d", prp->value_.num);
			else if (prp->name == "LPM_Width_Data_B")
				fprintf(fd, " Width_Data_B = %d", prp->value_.num);
			else if (prp->name == "LPM_Width_Scanout_A")
				fprintf(fd, " Width_Scanout_A = %d", prp->value_.num);
			else if (prp->name == "LPM_Width_Scanout_B")
				fprintf(fd, " Width_Scanout_B = %d", prp->value_.num);

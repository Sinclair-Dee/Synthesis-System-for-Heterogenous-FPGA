...
//ADU: NetDecoder
NetDecoder::NetDecoder(NetScope*s, perm_string name, unsigned awid)
	:NetNode(s, name, awid + (1U << awid)){
		awidth_ = awid;
		...
		for (unsigned idx = 0 ;  idx < ( 1U << awidth_) ;  idx += 1) {
...
Link& NetDecoder::pin_Result(unsigned r)
{
	assert(r < awidth_ + (1U << awidth_));
	return pin(awidth_ + r);
}

const Link& NetDecoder::pin_Result(unsigned r) const
{
	assert(r < awidth_ + (1U << awidth_));
	return pin(awidth_ + r);
}
...
NetMultAdd::NetMultAdd(NetScope*sc, perm_string n,
	...
	: NetNode(sc, n, 12 + w_dataa + w_datab + w_scanouta + w_scanoutb + w_result),
...
Link& NetMultAdd::pin_scanouta(unsigned idx)
{
	...
	return pin(idx+12+width_dataa_+width_datab_);
}
...
Link& NetMultAdd::pin_scanoutb(unsigned idx)
{
	...
	return pin(idx+12+width_dataa_+width_datab_+width_scanouta_);
}

const Link& NetMultAdd::pin_scanoutb(unsigned idx) const
{
	assert(idx < width_scanoutb_);
	return pin(idx+12+width_dataa_+width_datab_+width_scanoutb_);
}

Link& NetMultAdd::pin_result(unsigned idx)
{
	assert(idx < width_result_);
	return pin(idx+12+width_dataa_+width_datab_+width_scanouta_+width_scanoutb_);
}

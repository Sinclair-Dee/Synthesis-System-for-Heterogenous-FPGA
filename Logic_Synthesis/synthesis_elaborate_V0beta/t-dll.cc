...
void dll_target::lpm_decoder(const NetDecoder*net)
{
	...
	obj->u_.decoder.awidth = net->awidth();

	scope_add_lpm(obj->scope, obj);

	const Nexus*nex;

	assert(obj->u_.decoder.awidth);

	obj->u_.decoder.data = new ivl_nexus_t [obj->u_.decoder.awidth];

	for (unsigned idx = 0 ;  idx < obj->u_.decoder.awidth ;  idx += 1) {
		nex = net->pin_Data(idx).nexus();
		assert(nex->t_cookie());
	...
	obj->u_.decoder.result = new ivl_nexus_t [1U << obj->u_.decoder.awidth];

	for (unsigned idx = 0 ;  idx < (1U << obj->u_.decoder.awidth) ;  idx += 1) {
		nex = net->pin_Result(idx).nexus();
		assert(nex->t_cookie());
		obj->u_.decoder.result[idx] = (ivl_nexus_t) nex->t_cookie();
		nexus_lpm_add(obj->u_.decoder.result[idx], obj, 0,
			IVL_DR_HiZ, IVL_DR_HiZ);
	}
}

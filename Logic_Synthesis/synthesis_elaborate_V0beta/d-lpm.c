...
static void lpm_show_mip(ivl_lpm_t net)
{
	...
	if (cell == 0) {
		cell = edif_xcell_create(xlib, strdup(name),
			8 + ivl_lpm_mip_width(net,0) + ivl_lpm_mip_width(net,1) + ivl_lpm_mip_width(net,2)
			+ ivl_lpm_mip_width(net,3) + ivl_lpm_mip_width(net,4) + ivl_lpm_mip_width(net,5));

		sprintf(name, "clocken0");
		...
		edif_cell_pinteger(cell, "LPM_Width_Q_A", ivl_lpm_mip_width(net,4));
		edif_cell_pinteger(cell, "LPM_Width_Q_B", ivl_lpm_mip_width(net,5));
	}
...
static void lpm_show_mult_add(ivl_lpm_t net)
{
	...
	if (cell == 0) {
		cell = edif_xcell_create(xlib, strdup(name),
			12 + ivl_lpm_mult_width(net,0) + ivl_lpm_mult_width(net,1) + ivl_lpm_mult_width(net,2) + ivl_lpm_mult_width(net,3) + ivl_lpm_mult_width(net,4));
...
static void lpm_show_decoder(ivl_lpm_t net)
{
	...
	sprintf(name, "decoder%u_%u",
		ivl_lpm_decoder_awidth(net), 1U << ivl_lpm_decoder_awidth(net));
	cell = edif_xlibrary_findcell(xlib, name);
	...
	if (cell == 0) {
		...
		for (idx = 0 ;  idx < ivl_lpm_decoder_awidth(net) ;  idx += 1) {
			sprintf(name, "Data%u", idx);
			edif_cell_portconfig(cell, idx, strdup(name), IVL_SIP_INPUT);
		}

		for (idx = 0 ;  idx < (1U<<ivl_lpm_decoder_awidth(net));  idx += 1) {
			sprintf(name, "Result%u", idx);
			edif_cell_portconfig(cell, ivl_lpm_decoder_awidth(net)+idx, strdup(name), IVL_SIP_OUTPUT);
		}

		edif_cell_pstring(cell,  "LPM_Type",  "LPM_decoder");
		edif_cell_pinteger(cell, "LPM_aWidth", ivl_lpm_decoder_awidth(net));
	}
	...
	for (idx = 0 ;  idx < (1U<<ivl_lpm_decoder_awidth(net)); idx += 1) {
		...
		jnt = edif_joint_of_nexus(edf, ivl_lpm_decoder_result(net, idx));
		edif_add_to_joint(jnt, ref, pin);
	}

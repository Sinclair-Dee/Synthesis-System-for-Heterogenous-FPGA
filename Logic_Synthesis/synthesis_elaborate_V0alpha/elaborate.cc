...
void PMultAssign::elaborate(Design*des, NetScope*scope) const
{
	...
	connect(mult->pin_aclr(), eval[3]->pin(0));
	...
	for (unsigned idx = 0 ;  idx < eval[6]->pin_count() ; idx += 1)
		connect(mult->pin_result(idx), eval[6]->pin(idx));
}
...
void PMultAddAssign::elaborate(Design*des, NetScope*scope) const
{
	...
	NetMultAdd*mult_add = new NetMultAdd(scope, scope->local_symbol(),
		eval[0]->pin_count(),
		eval[1]->pin_count(),
		eval[14]->pin_count(),
		eval[15]->pin_count(),
		eval[16]->pin_count());
	des->add_node(mult_add);

	for (unsigned idx = 0 ;  idx < eval[0]->pin_count() ; idx += 1)
		connect(mult_add->pin_dataa(idx), eval[0]->pin(idx));
	for (unsigned idx = 0 ;  idx < eval[1]->pin_count() ; idx += 1)
		connect(mult_add->pin_datab(idx), eval[1]->pin(idx));
	connect(mult_add->pin_clock0(), eval[2]->pin(0));
	connect(mult_add->pin_clock1(), eval[3]->pin(0));
	connect(mult_add->pin_clock2(), eval[4]->pin(0));
	connect(mult_add->pin_clock3(), eval[5]->pin(0));
	connect(mult_add->pin_aclr0(), eval[6]->pin(0));
	connect(mult_add->pin_aclr1(), eval[7]->pin(0));
	connect(mult_add->pin_aclr2(), eval[8]->pin(0));
	connect(mult_add->pin_aclr3(), eval[9]->pin(0));
	connect(mult_add->pin_ena0(), eval[10]->pin(0));
	connect(mult_add->pin_ena1(), eval[11]->pin(0));
	connect(mult_add->pin_ena2(), eval[12]->pin(0));
	connect(mult_add->pin_ena3(), eval[13]->pin(0));
	for (unsigned idx = 0 ;  idx < eval[14]->pin_count() ; idx += 1)
		connect(mult_add->pin_scanouta(idx), eval[14]->pin(idx));
	for (unsigned idx = 0 ;  idx < eval[15]->pin_count() ; idx += 1)
		connect(mult_add->pin_scanoutb(idx), eval[15]->pin(idx));
	for (unsigned idx = 0 ;  idx < eval[16]->pin_count() ; idx += 1)
		connect(mult_add->pin_result(idx), eval[16]->pin(idx));
}

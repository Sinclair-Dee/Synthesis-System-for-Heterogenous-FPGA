...
bool Link::is_linkedToSignificantSignal() const
{
	...
	for (Link*cur = nex->first_nlink()
		; cur ;  cur = cur->next_nlink()) {
			...
			//if (cursig->port_type() != NetNet::NOT_A_PORT)//因为所有的port为“非内部信号”，则该条件多余2013.10.21——ADU
			//	return true;

			//if (cursig->port_type() == NetNet::PINPUT
			//	|| cursig->port_type() == NetNet::PINOUT
			//	|| cursig->port_type() == NetNet::POUTPUT)

			if (!cursig->local_flag())//2013.10.21"ivl_assemble_04" debug "fir_scu_rtl_restructured_for_cmm_exp.v"
				return true;          //有时候当程序执行到此处时SignificantSignal并没有连接到逻辑单元上，就会出现误判的情况，即当作非SignificantSignal来处理。
	}
...
int Nexus::is_driven() const
{
	...
	for (Link*cur = list_ ; cur ;  cur = cur->next_) {
		if (cur->get_dir() == Link::OUTPUT)
			count += 1;
		/*Driver is not only the output of the logic but also the 
		input pad*/
		if(const NetNet*temp = dynamic_cast<const NetNet*>(cur->get_obj())){
			if((temp->port_type() == NetNet::PINPUT || temp->port_type() == NetNet::PINOUT) && (temp->scope()->parent() == 0))//ADU:判断是否为顶层scope？
				count += 1;
		}

...
void pform_make_pmemassign(svector<PExpr*>*elist,
			      const char* fn,
			      unsigned lineno)
{
	PMemAssign*tmp = new PMemAssign(elist);
	pform_cur_module->add_gate(tmp);
	tmp->set_file(fn);
	tmp->set_lineno(lineno);
}

void pform_make_psquareassign(svector<PExpr*>*elist,
			      const char* fn,
			      unsigned lineno)
{
	PSquareAssign*tmp = new PSquareAssign(elist);
	pform_cur_module->add_gate(tmp);
	tmp->set_file(fn);
	tmp->set_lineno(lineno);
}

void pform_make_pmultassign(svector<PExpr*>*elist,
			      const char* fn,
			      unsigned lineno)
{
	PMultAssign*tmp = new PMultAssign(elist);
	pform_cur_module->add_gate(tmp);
	tmp->set_file(fn);
	tmp->set_lineno(lineno);
}

void pform_make_pmultaddassign(svector<PExpr*>*elist,
			      const char* fn,
			      unsigned lineno)
{
	PMultAddAssign*tmp = new PMultAddAssign(elist);
	pform_cur_module->add_gate(tmp);
	tmp->set_file(fn);
	tmp->set_lineno(lineno);
}

void pform_make_pmultaccumassign(svector<PExpr*>*elist,
			      const char* fn,
			      unsigned lineno)
{
	PMultAccumAssign*tmp = new PMultAccumAssign(elist);
	pform_cur_module->add_gate(tmp);
	tmp->set_file(fn);
	tmp->set_lineno(lineno);
}

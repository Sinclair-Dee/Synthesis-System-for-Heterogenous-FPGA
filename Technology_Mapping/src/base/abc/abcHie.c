...
void Abc_NtkFlattenLogicHierarchy_rec( Abc_Ntk_t * pNtkNew, Abc_Ntk_t * pNtk, int * pCounter, Vec_Str_t * vPref )
{
    ...
    // process the blackbox
	// 如果网络本身是一个BLACK_Network，那么就简单的复制一个新网络，即不打平
    if ( Abc_NtkHasBlackbox(pNtk) )
    {
        // duplicate the blackbox
        assert( Abc_NtkBoxNum(pNtk) == 1 );
        ...
        // connect blackbox fanins to the PI nets
        assert( Abc_ObjFaninNum(pObj->pCopy) == Abc_NtkPiNum(pNtk) );
		//此时，pNtk的PI/PO对应pNtkNew中的Node结点，此处将pBoxNew的BI/BO和Fanout0(Node)连接起来
        Abc_NtkForEachPi( pNtk, pTerm, i )
            Abc_ObjAddFanin( Abc_ObjFanin(pObj->pCopy,i), Abc_ObjFanout0(pTerm)->pCopy );
...
Abc_Ntk_t * Abc_NtkInsertNewLogic( Abc_Ntk_t * pNtkH, Abc_Ntk_t * pNtkL )
{
  ...
  //      Abc_NtkDupObj( pNtkNew, Abc_ObjFanout0(pNetL), 0 );
		...
		//if the driver net is exist while has no fanout, it means it's been built
		// from Bo as a fanout, it can still has an PO as fanout. While if the driver
		// net already has a fanout means it has appeared to drive a PO
		if(pNetL->pCopy) 
		{
			if(Abc_ObjFanout0(pNetL)->pCopy)
			{
				printf( "Error in Abc_NtkInsertNewLogic():  Primary output %s is repeated twice.\n", Abc_ObjName(pNetH) );
			    Abc_NtkDelete( pNtkNew );
			    return NULL;
			}
			else

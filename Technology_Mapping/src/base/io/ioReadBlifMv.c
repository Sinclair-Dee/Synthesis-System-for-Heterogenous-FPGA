...
static void Io_MvReadPreparse( Io_MvMan_t * p )
{
    ...
    for ( pCur = p->pBuffer; *pCur; pCur++ )
    {
        if ( *pCur == '\n' )
        {
            *pCur = 0;  //comment by jzh @2013-09-07 use '/0' to replace '\n' to cut file into lines to several strings
//            if ( *(pCur-1) == '\r' )
//                *(pCur-1) = 0;
            fComment = 0;
...
static int Io_MvParseLineSubckt( Io_MvMod_t * p, char * pLine )
{
    ...
    nEquals = Io_MvCountChars( pLine, '=' );// Using = to count how many port in subckt
...
*/
    ...
    { 
		...
		// pName是pModel的PI在BLIF中的名称，而pName2是该PI端口在pNtk网络下连接的线网
		// 名称，比如subckt inst_name Results=N0, 则pName=Result，而pName2=N0；
		// 而pName=NULL出现的情况是因为subckt并没有使用全部端口，某些在例化过程中没有写出
        pName2 = NULL;

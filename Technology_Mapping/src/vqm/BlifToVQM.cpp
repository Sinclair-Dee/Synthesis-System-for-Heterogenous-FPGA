...
void BlifToVQM::printVQM(void)
{
	...
	if (ocout.is_open()){
							...
							//tempOutportsPseudoInports.count(ito->second) == 0
								...
								if (!wirePrint[ito->second])
								{
									...
									ocout << "wire " 
										<< ((tempOutportsPseudoInports.count(ito->second) != 0) ? "\\" : "") 
										<< ito->second 
										<< ((tempOutportsPseudoInports.count(ito->second) != 0) ? "~Inst " : "")
										<< ";" << endl;
									...
									&& tempInportsPseudoOutports.count(rName) == 0)
								{
									if (!wirePrint[rName])
									{
										...
										ocout << "wire " 
											<< ((tempOutportsPseudoInports.count(rName) != 0) ? "\\" : "") 
											<< rName 
											<< ((tempOutportsPseudoInports.count(rName) != 0) ? "~Inst " : "")
											<< ";" << endl;
					...
					ito != tempInports.end(); ito++){
						if (ito->second.compare(""))
						{
							ocout << "    ." << ito->first << "(" 
								...
								<< ((tempOutportsPseudoInports.count(ito->second) != 0) ? "\\" : "")
								<< ito->second 
								<< (tempOutportsPseudoInports.count(ito->second) ? "~Inst " : "") << ")," << endl;
						}
				}
						...
						if (ito->second.compare(""))
						{
							if (!firstInportPrint)
							{
								ocout << "    ." << ito->first << "(" 
									//<< (wirePrint.count(ito->second) ? "\\" : "") 
									<< ((tempOutportsPseudoInports.count(ito->second) != 0) ? "\\" : "")
									<< ito->second << (tempOutportsPseudoInports.count(ito->second) ? "~Inst " : "") << ")";
								firstInportPrint = true;
						...
						if (!portName.compare("inclk"))
						{
							...
							if (ito->second.compare(""))
							{
								ocout << "    ." << ito->first << "(" 
									...
									//<< ((tempOutportsPseudoInports.count(ito->second) != 0) ? "~" : "")
									<< ((tempOutportsPseudoInports.count(ito->second) != 0) ? "\\" : "")
									<< ito->second << (tempOutportsPseudoInports.count(ito->second) ? "~Inst " : "") << ")," << endl;
							}
						}						
				}
		...
		map<string, string> &bufferPortInfo = topModel->getBufferPortInfo();
		...
		for(map<string, InPort *>::const_iterator it = tempOutportsPseudoInports.begin(); 
			it != tempOutportsPseudoInports.end(); it++){
				...
				if (it->second != NULL)
				{
					...
					if (bufferPortInfo.count(it->first) != 0)
					{
						ocout << "assign " << it->first << " = " << bufferPortInfo[it->first] << ";" << endl;
					} 
					else
					{
						ocout << "assign " << it->first << " = " 
							...
							(topModel->getOutportStatus().count(this->split(tempRealName, "[").at(0)) == 0? "" : "\\") : "")
							<< tempRealName 
							<< (topModel->getInportStatus().count(tempRealName) == 0? "~Inst " : "")
							<< ";" << endl;
					}
				}
		}
